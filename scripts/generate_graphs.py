...
def create_rating_recovery_graph():
    ...
    for rating in [2.0, 3.0, 4.0, 5.0]:
        ...
        ax.text(-0.3, rating, f'{rating}★', ha='right', va='center',
                fontsize=9, color='#757575')

    # 背景色で期間を分ける
    ax.axvspan(0, 3, alpha=0.1, color='green', label='_nolegend_')
    ax.axvspan(3, 6, alpha=0.15, color='red', label='_nolegend_')
    ax.axvspan(6, 12, alpha=0.1, color='blue', label='_nolegend_')

    # 期間ラベル
    ax.text(1.5, 5.2, '炎上前', ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle='round', facecolor='#C8E6C9', alpha=0.7))
    ax.text(4.5, 5.2, '炎上直後', ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle='round', facecolor='#FFCDD2', alpha=0.7))
    ax.text(9, 5.2, '回復期', ha='center', va='center', fontsize=10,
...
def create_response_time_impact_graph():
    ...
    # データ
    response_hours = np.array([1, 3, 6, 12, 24, 48, 72, 168])  # 時間
    satisfaction_rate = np.array([92, 88, 82, 75, 68, 58, 48, 35])  # 満足度%
...
def create_meo_ranking_improvement_graph():
    """MEO順位改善グラフ（返信率と順位の関係）"""
    fig, ax = plt.subplots(figsize=(12, 7))

    # 返信率とMEO順位のデータ
    response_rate = np.array([0, 20, 40, 60, 80, 100])
    meo_ranking = np.array([15, 12, 8, 5, 3, 2])  # 順位（低いほど良い）
    ...
    ax.set_xlim(-5, 105)
    ax.set_ylim(16, 0)  # Y軸を反転（順位が低いほど上）
    ax.grid(True, alpha=0.3, linestyle='--')
...
def create_review_response_roi_graph():
    ...
    # 左側：コスト vs 売上増加
    categories = ['対応なし', '月10件返信', '月30件返信', '全件返信']
    costs = [0, 2, 5, 10]  # 万円/月
    revenue_increase = [0, 15, 35, 60]  # 万円/月
    ...
    ax1.set_title('口コミ返信のコストと売上増加', fontsize=12, fontweight='bold')
    ax1.set_xticks(x)
    ...
    # 右側：ROI（投資対効果）
    roi = [0, 650, 600, 500]  # %（売上増加/コスト * 100）
...
def create_crisis_damage_comparison():
    ...
    # データ
    damage_categories = ['売上減少', '広告費増加', 'レピュテーション\nマネジメント費用',
                        '弁護士費用', '人件費\n（対応工数）']

    # 対応別の被害額（万円）
    no_response = [500, 200, 0, 100, 50]  # 対応なし
    bad_response = [400, 150, 50, 80, 80]  # 不適切な対応
    good_response = [100, 50, 100, 30, 100]  # 適切な対応
        ...
        for bar in bars:
            ...
            if height > 0:
                ax.text(bar.get_x() + bar.get_width()/2., height + 10,
                       f'{int(height)}万', ha='center', va='bottom', fontsize=9)
    ...
    ax.text(len(damage_categories) - 1 - width, total_no + 50,
            f'合計: {total_no}万円', ha='center', va='bottom',
            fontsize=10, fontweight='bold', color='#C62828',
            bbox=dict(boxstyle='round', facecolor='#FFCDD2', alpha=0.7))
    ax.text(len(damage_categories) - 1, total_bad + 50,
    ...
    ax.text(len(damage_categories) - 1 + width, total_good + 50,
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(damage_categories)
    ax.legend(loc='upper left', fontsize=11)
    ax.grid(axis='y', alpha=0.3, linestyle='--')

    # 注釈
    ax.text(len(damage_categories) / 2, -80,
            '※ 適切な対応をすることで、被害額を約67%削減できます（850万円 → 380万円）',
...
def create_review_volume_trend():
    ...
    before_policy = np.array([5, 4, 6, 5, 7, 8])  # 返信施策前
    after_policy = np.array([8, 12, 15, 18, 20, 22])  # 返信施策後
    ...
    # 施策開始を強調
    ax.annotate('返信施策開始', xy=(6, 20), xytext=(7.5, 23),
                arrowprops=dict(arrowstyle='->', color='#F44336', lw=2),
                fontsize=11, fontweight='bold', color='#F44336')

    # 増加を強調
    ax.annotate('口コミ件数\n2.75倍に増加', xy=(12, 22), xytext=(10, 25),
