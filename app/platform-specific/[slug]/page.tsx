...
export async function generateStaticParams() {
  const slugs = getArticleSlugs('platform-specific');
...
export async function generateMetadata({
  ...
  const article = await getArticleBySlug('platform-specific', slug);
  ...
  const articleUrl = `${siteUrl}/platform-specific/${slug}`;
...
export default async function PlatformSpecificPage({ params }: PageProps) {
  const { slug } = await params;
  const article = await getArticleBySlug('platform-specific', slug);
                ...
                <div className="card p-6">
                  ...
                  <div className="space-y-3">
                    {relatedArticles.map((related) => (
                      <Link
                        key={related.slug}
                        href={`/platform-specific/${related.slug}`}
