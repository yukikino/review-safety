...
export async function generateStaticParams() {
  const slugs = getArticleSlugs('mild-response');
...
export async function generateMetadata({
  ...
  const article = await getArticleBySlug('mild-response', slug);
...
export default async function MildResponsePage({ params }: PageProps) {
  const { slug } = await params;
  const article = await getArticleBySlug('mild-response', slug);
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8 py-6 md:py-8 max-w-7xl overflow-x-hidden">
          ...
          <div className="grid lg:grid-cols-[1fr_280px] gap-6 md:gap-8 overflow-x-hidden">
            ...
            <aside className="space-y-4 md:space-y-6">
              {/* ナビゲーション（固定） */}
              <div className="card p-4 md:p-6 lg:sticky lg:top-6">
                <h3 className="font-bold mb-3 md:mb-4 text-sm" style={{ color: 'var(--gray-900)' }}>
                  📖 ページ内ナビ
                </h3>
                <nav>
                ...
                <div className="card p-4 md:p-6">
                  ...
                  <div className="space-y-3">
                    {relatedArticles.map((related) => (
                      <Link
                        key={related.slug}
                        href={`/mild-response/${related.slug}`}
